...
from pva_work_flow.planning.candidate_rules import changed_variable_names, validate_candidate_constraints
...
def check_design_type_distribution(
    ...
    local_count = (
        counts.get("local_optimization", 0)
        + counts.get("single_factor_perturbation", 0)
        + counts.get("failure_factor_verification", 0)
        + counts.get("failure_rescue_verification", 0)
    )
    ...
    fcv_count = (
        counts.get("failure_cause_validation", counts.get("failure_verification", 0))
        + counts.get("failure_factor_verification", 0)
        + counts.get("failure_rescue_verification", 0)
    )
    if fcv_count > total_n:
        errors.append(f"Rule 6 FAIL: failure verification entries ({fcv_count}) exceed total round size ({total_n})")
...
def check_max_variable_changes(
    ...
    for entry in lineage_table:
        ...
        if dt in ("single_factor_perturbation",):
            ...
            continue
        if dt in ("failure_verification", "failure_factor_verification", "failure_rescue_verification", "baseline_reproduction"):
            limit = 0 if dt == "baseline_reproduction" else 1
