...
from pva_work_flow.core.utils import read_json, canonicalize_candidate_materials


MUCIN_NAMES = {"gastric mucin", "mucin"}


def _is_mucin(name: Any) -> bool:
    return str(name or "").strip().lower() in MUCIN_NAMES


def preserve_biopolymer_additives(candidate: dict) -> None:
    """Keep mucin-like biopolymers as additives/lubricants, not crosslinkers."""
    ...
    for additive in additives:
        if isinstance(additive, dict) and _is_mucin(additive.get("name")):
            additive["name"] = "Gastric Mucin"
            additive["role"] = "lubricant"
            if additive.get("wt_percent") not in (None, ""):
                mucin_wt = additive.get("wt_percent")

    crosslinker = formulation.get("crosslinker") or {}
    if isinstance(crosslinker, dict) and _is_mucin(crosslinker.get("name")):
        if mucin_wt in (None, ""):
            mucin_wt = crosslinker.get("wt_percent")
        formulation["crosslinker"] = {}

    if mucin_wt not in (None, "") and not any(
        isinstance(additive, dict) and _is_mucin(additive.get("name"))
        for additive in additives
    ):
        additives.append(
            {
    ...
    if isinstance(materials, list):
        for material in materials:
            if isinstance(material, dict) and _is_mucin(material.get("name")):
                material["name"] = "Gastric Mucin"
                material["role"] = "lubricant"
...
def materialize_constrained_candidates(
    ...
    for entry in table:
        ...
        preserve_biopolymer_additives(candidate)
