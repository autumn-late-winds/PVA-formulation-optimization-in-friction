...
from .utils import canonicalize_material_name
...
def validate_candidate_constraints(
    ...
    elif design_type == "failure_verification":
        if len(changed_names) > 1:
            errors.append(f"{cid}: failure_verification changed {len(changed_names)} variables, max 1")
        if parent and new_material_names(candidate, parent):
            errors.append(f"{cid}: failure_verification introduced new materials outside limited_exploration")
