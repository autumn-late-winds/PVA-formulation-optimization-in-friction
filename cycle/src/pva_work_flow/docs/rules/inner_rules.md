...
### 规则 13：薄凝胶浸泡/后处理敏感性（2026-06-24 更新）

- 当前项目使用的是**薄 PVA 水凝胶薄膜**（无冻融循环 freeze_thaw=0），对浸泡时间极为敏感
- 长时间的浸泡/后处理（>1h）会导致过度溶胀、收缩应力或网络软化，进而在 10N 载荷摩擦测试中破裂
- R2/R3 连续破裂（post_soak 包括 1h、4.5h、0.5h），说明旧的 0.25~1h 窗口仍可能偏长；下一轮优先验证 5 min
- **默认浸泡/后处理时间应缩短**：断裂后的新候选 post_soak 优先从 0.083h（5min）开始，而非 0.25~4h
- **排查轮应优先测试 5 min soak**：failure_factor_verification 中，若怀疑浸泡导致破裂，应测试 0.083h，而非重复测试 0.5h、0.25h 或更长的 4h
- **仅短时表面处理即可测试**：0.083h（5min）的去离子水润湿或 GA/HCl 后处理 + 吸干表面多余液体后直接测试，对于薄凝胶是当前优先救援选择
- **PVA 强度救援**：若整轮仍破裂，优先把 PVA 提高到 18~20 wt%，不要只调 GA/HCl 浓度
- **GA/HCl 语义**：戊二醛和盐酸不是当前 PVA 成胶的必需交联剂/催化剂；成胶后用一定浓度 GA/HCl 浸泡属于后处理，可减小水凝胶体积，也可能带来收缩破裂风险
- 例外：如果配方有冻融循环（FT≥1），网络经过物理交联强化，可以承受更长的浸泡时间
