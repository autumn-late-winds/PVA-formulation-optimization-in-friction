...
### 规则 13：薄凝胶浸泡敏感性（2026-06-14 新增）

- 当前项目使用的是**薄 PVA 水凝胶薄膜**（无冻融循环 freeze_thaw=0），对浸泡时间极为敏感
- 长时间的浸泡（>1h）会导致过度溶胀、网络软化，进而在 10N 载荷摩擦测试中破裂
- R2 全部 15 个候选破裂（post_soak 包括 1h 和 4.5h），大概率与浸泡过度有关，而非配方变量本身
- **默认浸泡时间应缩短**：新候选的 post_soak 优先从 0.25~1h 开始，而非 1~4h
- **排查轮应优先测试缩短 soak**：failure_factor_verification 中，若怀疑浸泡导致破裂，应测试 0.5h 或 0.25h，而非测试更长的 4h
- **仅表面润湿即可测试**：0.25h（15min）的去离子水表面润湿 + 吸干表面多余水分后直接测试，对于薄凝胶是完全可接受的
